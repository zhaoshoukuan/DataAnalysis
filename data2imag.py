        ...
        if phase == 'phase': 
            ...
            for i in range(num): 
                ...
                if n != 1:
                    ...
                    im0 = axes[i//2][i%2].pcolormesh(cols_,rows_,np.abs(s),cmap='jet')
                    im1 = axes[i][1].pcolormesh(cols_,rows_,np.angle(s),cmap='jet')
...
def slidePlot(fig,ax,var):
    ...
    sa = Slider(axa, 'a', -1, 1, valinit=a)
    sb = Slider(axb, 'b', -1000, 1000, valinit=b,valstep=0.001)
